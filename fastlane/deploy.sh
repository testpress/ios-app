...
EXECUTED_TASKS+=("Login Screen image generated")
...
# 5️⃣ Update GoogleService-Info.plist (validate first)
if [ ! -f "$GOOGLE_PLIST_PATH" ]; then
  echo "❌ Google plist not found at $GOOGLE_PLIST_PATH"
  exit 1
fi

fastlane update_google_service_plist source:"$GOOGLE_PLIST_PATH"
EXECUTED_TASKS+=("GoogleService-Info.plist updated")

# 6️⃣ Disable/Remove Zoom if feature flag is false
...
# 7️⃣ Update App Identity (bundle ID + display name)
