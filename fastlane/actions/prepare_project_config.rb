  ...
  module Actions
    class PrepareProjectConfigAction < Action
      def self.run(params)
        ...
        # 2. Env Overrides
        config["api_key_id"]     = ENV["APPLE_API_KEY_ID"]     || config["api_key_id"]     || "DDJ2YGJZCP"
        config["api_key_issuer"] = ENV["APPLE_API_KEY_ISSUER"] || config["api_key_issuer"] || "69a6de92-69ad-47e3-e053-5b8c7c11a4d1"
        config["apple_id"]       ||= ENV["APPLE_APP_ID"]         || "6753752664"
        config["scheme"]         ||= "Arise Medical Academy"
